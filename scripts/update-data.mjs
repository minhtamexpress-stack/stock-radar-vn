...
async function pooled(items,limit,fn){
  let cursor=0;const workers=Array.from({length:Math.min(limit,items.length)},async()=>{while(cursor<items.length){const i=cursor++;await fn(items[i],i)}});await Promise.all(workers);
}
const marketChecks=[];
await pooled(symbols,6,async s=>{
  let out;
  try{out=await yahoo(s);marketChecks.push({name:s,ok:out.price!=null,provider:'Yahoo Finance'})}
  catch(e){out={symbol:s,provider:'Yahoo Finance',price:null,previousClose:null,changePct:null,volume:null,timestamp:null,points:[],error:String(e.message||e),generatedAt:now()};marketChecks.push({name:s,ok:false,error:String(e.message||e),provider:'Yahoo Finance'})}
  await writeFile(new URL(`${safe(s)}.json`,OUT),JSON.stringify(out));
});
const contextChecks=[];
await pooled(stockSymbols,5,async s=>{
  let out;
  try{
    const data=await fetchJson(`https://vietstock.info/api/stocks/${encodeURIComponent(s)}`);
    out={symbol:s,provider:'Vietstock Public API',data,generatedAt:now()};
    contextChecks.push({name:`context:${s}`,ok:true,provider:'Vietstock Public API'});
  }catch(e){
    out={symbol:s,provider:'Vietstock Public API',data:null,error:String(e.message||e),generatedAt:now()};
    contextChecks.push({name:`context:${s}`,ok:false,error:String(e.message||e),provider:'Vietstock Public API'});
  }
...
const marketOk=marketChecks.filter(x=>x.ok).length;
await writeFile(new URL('health.json',OUT),JSON.stringify({ok:marketOk>=Math.max(8,Math.floor(symbols.length*.75)),marketOk,marketTotal:symbols.length,contextOk:contextChecks.filter(x=>x.ok).length,contextTotal:contextChecks.length,checks,failedSymbols,generatedAt:now()}));
