...
async function load(){
  ...
  state.rows=u.map((base,i)=>{
    ...
    return{symbol:base.symbol,item,m,ctx,held,rec,plan,label,reasons:[],holdingAdvice:null};
  });
...
function render(){
  ...
  const rows=state.rows,buys=rows.filter(r=>/MUA/.test(r.label)),top=buys[0]||rows[0],held=rows.filter(r=>r.held),valid=rows.filter(r=>r.m.price!=null);
  v.innerHTML=`<div class="topbar"><div><h1 class="title">Decision Desk V1.4</h1><div class="sub">Opportunity Radar • vùng mua • stop • target • lý do • Data Confidence • quản trị vị thế</div></div><button id="decisionRefresh" class="btn primary">Phân tích lại</button></div>
  <div class="grid g4"><div class="card"><div class="label">Ứng viên mua</div><div class="metric ${buys.length?'good':'warn'}">${buys.length}</div></div><div class="card"><div class="label">Top candidate</div><div class="metric ${top?cls(top.label):''}">${top?.symbol||'—'}</div><div class="small">${top?.label||'Chưa có'}</div></div><div class="card"><div class="label">Đang nắm giữ</div><div class="metric">${held.length}</div></div><div class="card"><div class="label">Opportunity Universe</div><div class="metric">${valid.length}/${rows.length}</div><div class="small">Mã có snapshot giá / tổng universe</div></div></div>
  ...
  <div class="section grid g2">${rows.slice(0,8).map(r=>`<div class="card decisionCard"><div class="kpiRow"><h2>${r.symbol}</h2><b class="${cls(r.label)}">${r.label}</b></div>${r.holdingAdvice?`<div class="small"><b>Danh mục:</b> ${esc(r.holdingAdvice)}</div>`:''}<div class="small"><b>Giá hiện tại:</b> ${fmt(r.m.price)} • <b>Vùng mua:</b> ${zone(r.plan.buyLow,r.plan.buyHigh)} • <b>Stop:</b> ${fmt(r.plan.stop)} • <b>T1/T2:</b> ${fmt(r.plan.target1)} / ${fmt(r.plan.target2)}</div><div class="decisionReasons">${r.reasons.map(x=>`<div>• ${esc(x)}</div>`).join('')}</div>${r.item._news.headlines?.length?`<div class="tiny">${r.item._news.headlines.map(x=>esc(x)).join('<br>')}</div>`:''}</div>`).join('')}</div>
  <div class="section card"><h2>Cách đọc vùng giá</h2><div class="small">Vùng mua/stop/target là kế hoạch định lượng từ giá, ATR và cấu trúc SMA/20 phiên. Không phải lệnh tự động. Fundamentals, Smart Money và Catalyst chỉ được cộng điểm khi snapshot có dữ liệu có cấu trúc; nếu thiếu, Confidence giảm và hệ thống ghi rõ thiếu dữ liệu.</div></div>`;
